...
Matrix * create_matrix(int size){
  ...
  if( ( tempMatrix = malloc( sizeof( Matrix ) ) ) == NULL ){
    fprintf(stderr, "Error in using malloc\n" );
    abort();
  }
  tempMatrix->size = size;
  if((tempMatrix->matrix = malloc(sizeof(float *) * size)) == NULL){
    fprintf(stderr, "Error in using malloc\n" );
    abort();
  }

  for(i = 0; i < size; i ++){
    if((tempMatrix->matrix[i] = malloc(sizeof(float) * size)) == NULL){
      fprintf(stderr, "Error in using malloc\n" );
      abort();   
    }
    for(j = 0; j < size; j ++){
